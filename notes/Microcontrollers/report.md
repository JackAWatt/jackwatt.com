...
The Zumo 32U4 robot is a small robot made to fit into the mini sumo (500-gram class and 10cm * 10cm). This robot comes both in a kit version and a pre-assembled version. We--our class--received the pre-assembled version. 

This robot comes with motors, proximity sensors, line sensors, an LCD, and a piezo buzzer. 

The operation of the sumo-bot is to use the proximity sensors to find opponents, when they find the opponent, they attempt to knock them out of the ring. The ring consists of a large black surface with a white edge. Using the line sensors the autonomous sumo-bot determines whether it has reached the white edge. When the white edge is found the bot will turn by some amount set by the programmer--usually a random value between two angles. 

Through basic programmable rules, such as when it finds an opponent it tries to knock them out of the ring, and when it sees the white edge, it knows it must back up or turn around, we end up with a fully autonomous robot. This acts as a demonstration of how the addition of simple rules can result in the emergence of what appears to be complex behavior. 
...
Located on the underside of the bot, these IR sensors read different values based on the contrast of the surface underneath the bot. The darker the surface the lower the value the IR sensor will read. When on a white surface the sensors read close to the maximum which is 2^10, and when on a black surface they read close to the minimum which is 0. Actual read values will vary based on each sensor and the surface contrast and material. 

* Proximity Sensors

Two IR sensors are mounted on the front of the bot, which are used to detect nearby objects or opponents. These are used during competition to seek for opponents to attack

* (IMU) - Accelerometer, Gyroscope, and Magnetometer

This is a 9-axis inertial measurement unit that will detect motion, orientation, and acceleration. This is used for advanced navigation tasks, although we didn't use the IMU in this project. 
...
This always each motor to be driven separately, allowing for differential steering--turning by varying the speed of each motor, one motor could be run slower than the other, or even in the opposite direction.

* PWN speed control: 

The motors are controlled by pulse-width modulation from the microcontroller, allowing precise speed and direction adjustment. 

* H-bridge: 

This is an integrated motor driver that acts as protection to the microcontroller. It allows for a a load current to the motors that the microcontroller couldn't handle. Essentially the microcontroller sends a signal to the hbridge which allows the current delivered to the motors to be drawn from the battery directly rather than through a pin on the microcontroller. 

### Other parts

* Piezo Buzzer 

This buzzer can produce varying tones, which can be used as alerts or simple melodies. It was used in the competition to signal that the sumo bot was activated and the countdown before it started its routine began. 
...
Early on we found that the bot would drift slightly to one side. We fixed this by slowing down one motor and speeding the other one up. We tested the drift by programming the bot to turn in place by 90 degrees. As we calibrated the motor alignment, we could get the bot to turn in place by 90 degrees multiple times without much drift. 
...
When testing the proximity sensors, we found that some varied by a large amount. When we tested with an object in front of the bot, we found that the left sensor value was 50% more than the right sensor value, but that the left front and right front read the same value. 

When testing the reflective contrast sensors, we found that the sensor read almost 100% more on sensor 5 than on sensor 1. During other tests the variance was less, but still enough that would lead one to believe that each sensor were seeing different contrasts, when at most there could be some minor surface damage to the platform we used. 
...
Use the serial monitor in Arduino studio to print sensor values while testing. 
...
The values given to us were a turning speed of 200 and a turning duration 300. We increased the turning speed and decreased the turning duration so it would more in a shorter period. In total the bot's turning radius has been reduced because we found that the default behavior of the bot had it turning so much when it found the edge that sometimes it would immediately run back into the edge and then be put back into the turning routine. 

* #define SEARCH_SPEED 400

The value given to us was 200. We increased this to the max because while searching if the bot finds an opponent, then it will be at its max speed and because F = MA, we know that the force we'll hit the opponent with is at it's maximum when the acceleration is also at it's maximum. 

* #define STOP_DURATION 200

The default stop during given was 100. We increased it to create a smoother turning or backing up motion. This allows the bot to settle before trying to move. Of all the changes we made, this one is the one I'm the least confident in whether it benefited the operation of our bot, but in theory it seems to be a good change.
